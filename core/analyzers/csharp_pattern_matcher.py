...
SINK_PATTERNS: list[SinkPattern] = [
    ...
    SinkPattern(
        pattern_id="XXE-001B",
        name="XDocument.Load sin XmlReader seguro",
        symbol_prefix="System.Xml.Linq.XDocument.Load",
        vulnerability_kind=VulnerabilityKind.XXE,
        tainted_arg_positions=(0,),
        always_vulnerable=False,
        confidence=TaintConfidence.MEDIUM,
        description="XDocument.Load con XML externo sin XmlReader seguro puede ser vulnerable a XXE.",
        remediation="Usar XDocument.Load(XmlReader.Create(stream, safeSettings)) con DtdProcessing=Prohibit.",
        cwe="CWE-611",
        cvss_base=5.9,
    ),
    SinkPattern(
